...
function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } } // note on window.msal usage. There is little point holding the object constructed by new Msal.UserAgentApplication
...
function loginAndAcquireToken(successCallback) {
	var localMsalApp = window.msal;
	var scopes = [].concat(_toConsumableArray(appConfig.scopes), ['openid', 'profile']);
	var account = localMsalApp.getAccount();
	...
	if (!account) {
		...
		if (state.noScopes) {
			...
			else localMsalApp.loginRedirect({
					scopes: scopes
				});
		} else {
			// try to get token from SSO session
			localMsalApp.acquireTokenSilent({
				scopes: scopes
				...
				if (error) {
					if (appConfig.silentLoginOnly) state.errorApp();else localMsalApp.loginRedirect({
						scopes: scopes
					});
				}
			});
...
var authentication = {
	...
	initialize: function initialize(config) {
		...
		var authority = 'https://' + instance + '/' + config.tenant + '/' + config.signInPolicy;
		...
		state.scopes = [].concat(_toConsumableArray(scopes), ['openid', 'profile']);
